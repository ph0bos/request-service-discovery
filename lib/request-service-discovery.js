...
var RETRY_DEFAULTS   = { retries: 3, minTimeout: 75, maxTimeout: 1000 };
...
RequestServiceDiscovery.prototype._request = function(method, uri, headers, body, callback) {
  ...
  operation.attempt(function(currentAttempt) {
    ...
    if (self.options.verbose) {
      _log(self.options.serviceName, method, uri, headers, body, 'resolving service instance');
    }
    ...
    _getServiceInstance(function(err, data) {
      ...
      if (self.options.verbose) {
        _log(self.options.serviceName, method, data.serviceUrl + '/' + uri, headers, body, 'performing request');
      }
      ...
      _end(request, function (err, res) {
        ...
        if (self.options.verbose) {
          _log(self.options.serviceName, method, data.serviceUrl + '/' + uri, headers, body, 'request complete');
        }
...
function _isZookeeperConnected() {
  return zoologist.getClient().isConnected();
}
...
/**
 * Internal logging proxy
 */
function _log(service, method, uri, headers, body, msg) {
  log.info({
    serviceName: service,
    method: method,
    url: uri,
